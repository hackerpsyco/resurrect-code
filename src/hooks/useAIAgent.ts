import { useState, useCallback } from "react";
import { supabase } from "@/integrations/supabase/client";
import { toast } from "sonner";

interface ErrorInfo {
  deploymentId: string;
  projectName: string;
  branch: string;
  commitMessage: string;
  errorMessage: string;
  errorLogs: string[];
  owner?: string;
  repo?: string;
}

interface AgentStep {
  id: string;
  name: string;
  status: "pending" | "running" | "completed" | "error";
  result?: unknown;
  timestamp?: string;
}

interface PRResult {
  html_url?: string;
  number?: number;
}

interface FileChange {
  path: string;
  oldContent: string;
  newContent: string;
}

interface FixResult {
  prTitle?: string;
  prDescription?: string;
  changes?: Array<{
    file: string;
    action: string;
    content: string;
  }>;
}

export function useAIAgent() {
  const [isRunning, setIsRunning] = useState(false);
  const [steps, setSteps] = useState<AgentStep[]>([]);
  const [currentStep, setCurrentStep] = useState<string | null>(null);
  const [pendingChanges, setPendingChanges] = useState<FileChange[]>([]);
  const [pendingPRInfo, setPendingPRInfo] = useState<{ title: string; description: string } | null>(null);

  const updateStep = (stepId: string, updates: Partial<AgentStep>) => {
    setSteps((prev) =>
      prev.map((s) => (s.id === stepId ? { ...s, ...updates } : s))
    );
  };

  const runAgent = useCallback(async (errorInfo: ErrorInfo) => {
    setIsRunning(true);
    setSteps([
      { id: "analyze", name: "Analyzing Error", status: "pending" },
      { id: "search", name: "Searching Solutions", status: "pending" },
      { id: "generate", name: "Generating Fix", status: "pending" },
      { id: "pr", name: "Creating PR", status: "pending" },
    ]);

    try {
      // Step 1: Analyze error
      setCurrentStep("analyze");
      updateStep("analyze", { status: "running" });
      
      const { data: analyzeData, error: analyzeError } = await supabase.functions.invoke(
        "ai-agent",
        {
          body: {
            action: "analyze_error",
            errorInfo,
          },
        }
      );

      if (analyzeError) throw new Error(analyzeError.message);
      
      updateStep("analyze", {
        status: "completed",
        result: analyzeData.result,
        timestamp: new Date().toISOString(),
      });
      
      toast.info("Error analyzed", {
        description: analyzeData.result?.rootCause || "Analysis complete",
      });

      // Step 2: Search solutions
      setCurrentStep("search");
      updateStep("search", { status: "running" });

      const { data: searchData, error: searchError } = await supabase.functions.invoke(
        "ai-agent",
        {
          body: {
            action: "search_solution",
            errorAnalysis: JSON.stringify(analyzeData.result),
          },
        }
      );

      if (searchError) throw new Error(searchError.message);

      updateStep("search", {
        status: "completed",
        result: searchData.result,
        timestamp: new Date().toISOString(),
      });

      const solutionCount = searchData.result?.solutions?.length || 0;
      toast.info(`Found ${solutionCount} solution(s)`, {
        description: "Generating fix code...",
      });

      // Step 3: Generate fix
      setCurrentStep("generate");
      updateStep("generate", { status: "running" });

      const { data: fixData, error: fixError } = await supabase.functions.invoke(
        "ai-agent",
        {
          body: {
            action: "generate_fix",
            errorAnalysis: JSON.stringify(analyzeData.result),
            searchResults: JSON.stringify(searchData.result),
          },
        }
      );

      if (fixError) throw new Error(fixError.message);

      updateStep("generate", {
        status: "completed",
        result: fixData.result,
        timestamp: new Date().toISOString(),
      });

      // Prepare file changes for diff view
      const fixResult = fixData.result as FixResult;
      const owner = errorInfo.owner || errorInfo.projectName.split("/")[0];
      const repo = errorInfo.repo || errorInfo.projectName.split("/")[1] || errorInfo.projectName;

      if (fixResult.changes && fixResult.changes.length > 0) {
        const fileChanges: FileChange[] = [];
        
        for (const change of fixResult.changes) {
          try {
            // Fetch original file content
            const { data: fileData } = await supabase.functions.invoke("github-api", {
              body: {
                action: "get_file",
                owner,
                repo,
                path: change.file,
                branch: errorInfo.branch,
              },
            });

            fileChanges.push({
              path: change.file,
              oldContent: fileData?.data?.decodedContent || "",
              newContent: change.content,
            });
          } catch (error) {
            console.warn(`Could not fetch original content for ${change.file}:`, error);
            fileChanges.push({
              path: change.file,
              oldContent: "",
              newContent: change.content,
            });
          }
        }

        setPendingChanges(fileChanges);
        setPendingPRInfo({
          title: fixResult.prTitle || `[ResurrectCI] Fix build error in ${errorInfo.branch}`,
          description: fixResult.prDescription || `Automated fix generated by ResurrectCI AI Agent.\n\nError: ${errorInfo.errorMessage}`,
        });

        // Return early - wait for user confirmation
        setIsRunning(false);
        return { 
          success: true, 
          fix: fixData.result, 
          requiresConfirmation: true,
          owner,
          repo,
          branch: errorInfo.branch,
        };
      }

      // Step 4: Create real PR via GitHub API
      setCurrentStep("pr");
      updateStep("pr", { status: "running" });

      // Create branch and PR
      const { error: prError } = await supabase.functions.invoke(
        "github-api",
        {
          body: {
            action: "create_branch",
            owner,
            repo,
            baseBranch: errorInfo.branch,
            newBranch: "resurrect-fix",
          },
        }
      );

      if (prError) {
        console.warn("Branch creation warning:", prError.message);
      }

      // Create the PR (reuse fixResult from above)
      const { data: pullRequest, error: pullError } = await supabase.functions.invoke(
        "github-api",
        {
          body: {
            action: "create_pr",
            owner,
            repo,
            title: fixResult?.prTitle || `[ResurrectCI] Fix build error in ${errorInfo.branch}`,
            body: fixResult?.prDescription || `Automated fix generated by ResurrectCI AI Agent.\n\nError: ${errorInfo.errorMessage}`,
            baseBranch: errorInfo.branch,
            newBranch: "resurrect-fix",
          },
        }
      );

      const prResult: PRResult = pullRequest?.data || {};
      
      updateStep("pr", {
        status: pullError ? "error" : "completed",
        result: {
          prUrl: prResult.html_url || `https://github.com/${owner}/${repo}/pull/${prResult.number || "new"}`,
          branch: "resurrect-fix",
        },
        timestamp: new Date().toISOString(),
      });

      if (pullError) {
        toast.warning("PR creation had issues", {
          description: "Branch created but PR may need manual creation.",
        });
      } else {
        toast.success("Fix ready!", {
          description: "Pull request created on branch 'resurrect-fix'",
        });
      }

      setCurrentStep(null);
      return { success: true, fix: fixData.result };

    } catch (error) {
      console.error("AI Agent error:", error);
      const errorMessage = error instanceof Error ? error.message : "Unknown error";
      
      if (currentStep) {
        updateStep(currentStep, { status: "error" });
      }

      // Handle rate limiting
      if (errorMessage.includes("429") || errorMessage.includes("Rate limit")) {
        toast.error("Rate limit exceeded", {
          description: "Please wait a moment and try again.",
        });
      } else if (errorMessage.includes("402")) {
        toast.error("AI credits exhausted", {
          description: "Please add credits to continue using AI features.",
        });
      } else {
        toast.error("Agent failed", {
          description: errorMessage,
        });
      }

      return { success: false, error: errorMessage };
    } finally {
      setIsRunning(false);
    }
  }, [currentStep]);

  const confirmAndCreatePR = useCallback(async (owner: string, repo: string, branch: string) => {
    if (!pendingPRInfo || pendingChanges.length === 0) {
      toast.error("No pending changes to commit");
      return { success: false };
    }

    setIsRunning(true);
    setCurrentStep("pr");
    updateStep("pr", { status: "running" });

    try {
      // Create branch
      await supabase.functions.invoke("github-api", {
        body: {
          action: "create_branch",
          owner,
          repo,
          baseBranch: branch,
          newBranch: "resurrect-fix",
        },
      });

      // Update files
      for (const change of pendingChanges) {
        await supabase.functions.invoke("github-api", {
          body: {
            action: "update_file",
            owner,
            repo,
            path: change.path,
            content: change.newContent,
            message: `fix: update ${change.path}`,
            branch: "resurrect-fix",
          },
        });
      }

      // Create PR
      const { data: pullRequest, error: pullError } = await supabase.functions.invoke("github-api", {
        body: {
          action: "create_pr",
          owner,
          repo,
          title: pendingPRInfo.title,
          body: pendingPRInfo.description,
          baseBranch: branch,
          newBranch: "resurrect-fix",
        },
      });

      const prResult: PRResult = pullRequest?.data || {};

      updateStep("pr", {
        status: pullError ? "error" : "completed",
        result: {
          prUrl: prResult.html_url || `https://github.com/${owner}/${repo}/pull/${prResult.number || "new"}`,
          branch: "resurrect-fix",
        },
        timestamp: new Date().toISOString(),
      });

      if (pullError) {
        toast.warning("PR creation had issues", {
          description: "Branch created but PR may need manual creation.",
        });
      } else {
        toast.success("Fix ready!", {
          description: "Pull request created successfully",
        });
      }

      setPendingChanges([]);
      setPendingPRInfo(null);
      setCurrentStep(null);

      return { success: true, prUrl: prResult.html_url };
    } catch (error) {
      console.error("PR creation error:", error);
      const errorMessage = error instanceof Error ? error.message : "Unknown error";
      updateStep("pr", { status: "error" });
      toast.error("Failed to create PR", { description: errorMessage });
      return { success: false, error: errorMessage };
    } finally {
      setIsRunning(false);
    }
  }, [pendingPRInfo, pendingChanges]);

  return {
    runAgent,
    confirmAndCreatePR,
    isRunning,
    steps,
    currentStep,
    pendingChanges,
    pendingPRInfo,
  };
}
